...
- **自然语言搜索**（第 2 层 · 第二阶段）：同一 CLIP 空间文本 embedding（`encode_text`）→ `search_by_text`；Spotlight 全局搜索面板新增 **🧠 语义（CLIP）** 分区（索引为空时后台自动构建并在完成后自动刷新结果）；**首次加载模型后台预热**（不阻塞界面，完成后自动刷新结果），权重**离线优先**（`HF_HUB_OFFLINE`，避免网络受限时重试 15s+；需要联网下载新模型设 `VISUAL_SEARCH_ALLOW_DOWNLOAD=1`）；**中文关键词映射**（`core/visual_search/query.py`）：中文查询偏弱（实测「兽装」最高 0.220，英文 "a photo of fursuit" 0.318），
  检索时把中文关键词扩展成英文提示词，与原文一起编码、逐照片取最大相似度（**只升不降**；「风景/食物/夜晚」等英文反而更弱时保持原结果）；
  同一查询的文本向量带缓存（面板重渲染 / 扩展变体不重复编码）；只看收藏时先多取候选再筛（避免被收藏外结果占满）
...
- **索引自动维护**：分析/扫描新照片入库完成后，后台增量补齐视觉搜索索引（设置中心可关：`data.auto_update_visual_index`，失败只提示不弹窗）；设置中心「AI 数据」显示索引状态（已索引/总数/模型/更新时间）并提供「更新/重建视觉索引」入口（重建需二次确认，仅删搜索缓存）；**索引自清理**：更新时自动剔除已删除照片的条目（避免语义搜索返回打不开的结果、
  状态行数字漂移，以及“同内容重新导入却永不入索引”），状态行提示「含 N 张已删除」
...
`tests/` 共 **40 个测试文件 / 388 项**，全绿（运行结束进程退出码 0）；其中 2 个文件需要加载模型
